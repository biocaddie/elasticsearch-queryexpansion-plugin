...
public class Rocchio {
    private static final Logger staticLogger = ESLoggerFactory.getLogger(Rocchio.class);

    // FIXME: These are just random guesses see NDS-958
    private static final int ALPHA_BETA_MIN = 0;
    private static final int ALPHA_BETA_MAX = 1;
    private static final int K1_MIN = 0;
    private static final int K1_MAX = 2;
    private static final int B_MIN = 0;
    private static final int B_MAX = 1;

    // Error Strings returned from validate()
    public static final String NULL_INDEX_ERROR = "You must specify an index to expand against";
    public static final String NULL_QUERY_ERROR = "You must specify a query to expand";
    public static final String NULL_TYPE_ERROR = "You must specify a type";
    public static final String NULL_FIELD_ERROR = "You must specify a field";
    public static final String INVALID_FB_TERMS_ERROR = "Number of feedback terms (fbTerms) must be a positive integer";
    public static final String INVALID_FB_DOCS_ERROR = "Number of feedback documents (fbDocs) must be a positive integer";
    public static final String INVALID_ALPHA_ERROR = "Alpha value must be a real number between " + ALPHA_BETA_MIN + " and " + ALPHA_BETA_MAX;
    public static final String INVALID_BETA_ERROR = "Beta value must be a real number between " + ALPHA_BETA_MIN + " and " + ALPHA_BETA_MAX;
    public static final String INVALID_K1_ERROR = "K1 value must be a real number between " + K1_MIN + " and " + K1_MAX;
    public static final String INVALID_B_ERROR = "B value must be a real number between " + B_MIN + " and " + B_MAX;
    
    // Read default stoplist from src/main/resource/stoplist.all
    private static final Stopper DEFAULT_STOPPER;
    private static final String STOPLIST_NAME = "stoplist.all";
    static {
        Stopper stopper = new Stopper();
        
        ClassLoader loader = Rocchio.class.getClassLoader();
        try (BufferedReader br = new BufferedReader(new InputStreamReader(loader.getResourceAsStream(STOPLIST_NAME))))  {
            String curr = null;
            while ((curr = br.readLine()) != null) {
                stopper.addStopword(curr);
            }
        } catch (IOException e) {
           staticLogger.error(String.format("%s was not found.. continuing without a stoplist", STOPLIST_NAME), e);
        }
        
        DEFAULT_STOPPER = stopper;
    }
    ...
    private final Client client;      // ElasticSearch client
    private final String index;       // ElasticSearch index name
    private final String type;        // Document type
    private final String field;       // Field

    private final double alpha;       // Rocchio alpha
    private final double beta;        // Rocchio beta
    private final double k1;          // BM25 k1
    private final double b;           // BM25 b
    ...
    private void fail(String errorMessage) {
        staticLogger.error(errorMessage);
        throw new IllegalStateException(errorMessage);
    }

    private void failIf(Supplier<Boolean> condition, String errorMessage) {
        if (condition.get()) {
            staticLogger.error("Condition failed: " + condition.toString());
            fail(errorMessage);
        }
    }
    ...
    /**
     * Returns an error message if term vectors are misconfigured. Otherwise, returns null.
     * 
     * TODO: Some of this could potentially be called at plugin startup, if we know what index/type we plan to expand
    ...
    private String ensureTermVectors() throws IOException {
        ...
        // TODO: NDS-958 - Check that term vectors/fields stats are available for the desired index/type/field combination?

        // If neither of the above triggered, then we didn't have the right term vectors initialized on our index
    ...
    /**
     * Run the query using the client (this assumes that the client has already been initialized and is ready to
     * execute)
     * 
     * @param index
     ...
     * @throws IOException
     *             if the TermVector has no fields, or if its Fields contain no terms
     */
    ...
    /**
     * Debug: this main method will run Rocchio as a standalone command-line application.
     * 
     * NOTE: You will need to add the following dependency to your {@code pom.xml}:
     * 
     * <pre>
     *  &lt;dependency&gt;
     ...
     * @throws IOException
     *             if expandQuery throws an IOException, or if the host lookup fails (localhost shouldn't)
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        /*
         * Options options = createOptions(); CommandLineParser parser = new GnuParser(); CommandLine cl =
         * parser.parse(options, args); if (cl.hasOption("help")) { HelpFormatter formatter = new HelpFormatter();
         * formatter.printHelp(Rocchio.class.getCanonicalName(), options); return; }
         * 
         * // Get the many command line parameters String cluster = cl.getOptionValue("cluster", "elasticsearch");
         * String host = cl.getOptionValue("host", "localhost"); int port = Integer.parseInt(cl.getOptionValue("port",
         * "9300")); double alpha = Double.parseDouble(cl.getOptionValue("alpha", "0.5")); double beta =
         * Double.parseDouble(cl.getOptionValue("beta", "0.5")); double k1 = Double.parseDouble(cl.getOptionValue("k1",
         * "1.2")); double b = Double.parseDouble(cl.getOptionValue("b", "0.75")); int fbTerms =
         * Integer.parseInt(cl.getOptionValue("fbTerms", "10")); int fbDocs =
         * Integer.parseInt(cl.getOptionValue("fbDocs", "10")); String index = cl.getOptionValue("index", "biocaddie");
         * String type = cl.getOptionValue("type", "dataset"); String field = cl.getOptionValue("field", "_all");
         * 
         * String auth = cl.getOptionValue("auth", "elastic:biocaddie"); String query = cl.getOptionValue("query",
         * "multiple sclerosis");
         * 
         * // Connect to ElasticSearch Settings settings = Settings.builder().put("cluster.name", cluster).build();
         ...
         * 
         * // Construct Rocchio Rocchio rocchio = new Rocchio(client, index, type, field, alpha, beta, k1, b);
         * 
         * // Expand the query FeatureVector feedbackQuery = rocchio.expandQuery(query, fbDocs, fbTerms);
         * 
         * // Dump the expanded query StringBuffer esQuery = new StringBuffer(); for (String term :
         * feedbackQuery.getFeatures()) { esQuery.append(term + "^" + feedbackQuery.getFeatureWeight(term) + " "); }
