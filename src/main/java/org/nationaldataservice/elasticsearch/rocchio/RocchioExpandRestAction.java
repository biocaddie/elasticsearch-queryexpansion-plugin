...
public class RocchioExpandRestAction extends BaseRestHandler {
     ...
     * 
     * @param index the String index to expand against
     * @param query the String query to expand
     * @param type the String type within the index
     * @param field the String field on the type
     * @param fbDocs the int number of feedback documents
     * @param fbTerms the int number of feedback terms
     * @param alpha the double Rocchio alpha parameter
     * @param beta the double Rocchio beta parameter
     * @param k1 the double Rocchio k1 parameter
     * @param b the double Rocchio b parameter
     * @return the String error message, or null if no errors are encountered
     */
    private String getErrors(String index, String query, String type, 
    						String field, int fbDocs, int fbTerms, 
    ...
    /**
     * Returns an error message if term vectors are misconfigured.
     * Otherwise, returns null.
     * 
     * TODO: Some of this can potentially be called at plugin startup, 
     *       if we know what index/type we plan to expand against 
     *       ahead of time...
     * 
     * @param client the client to use for the connection
     * @param index the index to check for the desired type
     * @param type the type to check for the desired field
     * @param field the field for which to verify that term vectors are enabled
     * @return the String error message, or null if no errors are encountered
     * 
    ...
    private String ensureTermVectors(Client client, String index, String type, String field) throws IOException {
    	...
    	LinkedHashMap<String, Object> fieldProperties, allFieldProperties = (LinkedHashMap<String, Object>) typeMap.get("_all");
		...
		if (allFieldProperties.containsKey("store")) {
			...
			if (!storeEnabled) {
				this.logger.error("Term vectors storage for on " + index + "." + type + "." + field + " has been disabled");
			...
			if (!storeEnabled) {
				this.logger.error("Term vectors storage for on index " + index + "." + type + "." + field + " has been disabled");
				return "Term vectors storage for " + index + "." + type + "." + field + " has been disabled";
			}
		...
		this.logger.error("Term vectors storage for on index " + index + "." + type + "." + field + " has not been configured");
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
    	...
    	this.logger.info(String.format("Starting Rocchio (%s,%s,%s,%s,%d,%d,%.2f,%.2f,%.2f,%.2f)", index, query, type, field, fbDocs, fbTerms, alpha, beta, k1, b));
    	...
    	// Examine the index to verify that store == true for the desired index/type/field combination
    	shortCircuit = this.ensureTermVectors(client, index, type, field);
    	if (!Strings.isNullOrEmpty(shortCircuit)) {
    		return throwError(shortCircuit);
    	}
    	
    	// TODO: Check that term vectors/fields stats are available for the desired index/type/field combination?
